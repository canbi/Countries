...
//
//  Updated by Can Bi on 06.04.2022.
//
...
class LocalFileManagerImage: LocalFileManager, ObservableObject {
    ...
    func saveImage(svgImage: Data, name: String) -> String {
        guard let path = getPathFor(name: name, type: type) else {
            return "Error getting data."
        }
        
        do {
            try svgImage.write(to: path, options: [.atomic, .completeFileProtection])
